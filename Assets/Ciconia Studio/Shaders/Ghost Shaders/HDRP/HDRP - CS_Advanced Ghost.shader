Shader "Ciconia Studio/CS_Ghost/HDRP/CS_Advanced Ghost"
{
	Properties
	{
		...
		[HDR][Space(15)][Header(_____ Fresnel Properties _____)][Space(15)] _FresnelColor( "Color", Color ) = ( 0.6933962, 1, 0.9814353, 1 )
		...
		[Space(35)][Header(Details Properties________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________)][Space(15)] _DetailMap( "Detail Map", 2D ) = "white" {}
		...
		[HDR][Space(35)][Header(Transparency Properties________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________)][Space(15)] _FillColorBackground( "Fill Color Background", Color ) = ( 0, 0, 0, 0 )
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 texCoord1_g14 = UV0.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float2 uv_BumpMap = UV0.xy * _BumpMap_ST.xy + _BumpMap_ST.zw;
				...
				float2 uv_DetailMap = UV0.xy * _DetailMap_ST.xy + _DetailMap_ST.zw;
				float TexturesScale451 = _TexturesScale;
				float4 unityObjectToClipPos420 = TransformWorldToHClip( TransformObjectToWorld( ( packedInput.ase_texcoord6.xyz ).xyz ) );
				...
				float4 staticSwitch480 = float4( temp_output_271_0 , 0.0 );
				#endif
				float4 fetchOpaqueVal15 = ASEHDSampleSceneColor(( (ase_grabScreenPosNorm).xyzw + (( staticSwitch480 *  (-1.0 + ( _Refraction - 0.0 ) * ( 1.0 - -1.0 ) / ( 2.0 - 0.0 ) ) )).rgba ).xy.xy, 0.0, GetInverseCurrentExposureMultiplier());
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float clampResult2_g11 = clamp( _FresnelTransIntensity , 0.0 , 400.0 );
				float fresnelNdotV5_g11 = dot( ase_normalWS, ase_viewDirWS );
				float fresnelNode5_g11 = ( _FresnelTransBias + clampResult2_g11 * pow( max( 1.0 - fresnelNdotV5_g11 , 0.0001 ), _FresnelTransPower ) );
				float temp_output_7_0_g11 = saturate( ( 1.0 - fresnelNode5_g11 ) );
				float OpacityFresnelSub743 = temp_output_7_0_g11;
				...
				float4 unityObjectToClipPos437 = TransformWorldToHClip( TransformObjectToWorld( ( float3( 0, 0, 0 ) ).xyz ) );
				float4 computeScreenPos438 = ComputeScreenPos( unityObjectToClipPos437 , _ProjectionParams.x );
				float4 transform428 = mul(GetObjectToWorldMatrix(),float4( ase_positionWS , 0.0 ));
				transform428.xyz = GetAbsolutePositionWS((transform428).xyz);
				float4 WorldProjection345 = ( ( ( ( computeScreenPos421 / (computeScreenPos421).w ) - ( computeScreenPos438 / (computeScreenPos438).w ) ) * _TexturesScale ) * distance( ( float4( _WorldSpaceCameraPos , 0.0 ) - transform428 ) , float4( 0,0,0,0 ) ) );
				#if defined( _UVSCREENPROJECTION_UVPROJECTION )
				float4 staticSwitch394 = float4( ( uv_DetailMap * TexturesScale451 ), 0.0 , 0.0 );
		...
		Pass
		{
					 ...
					 )
			{
				...
				float3 ase_normalWS = packedInput.ase_texcoord4.xyz;
				float clampResult2_g11 = clamp( _FresnelTransIntensity , 0.0 , 400.0 );
				float fresnelNdotV5_g11 = dot( ase_normalWS, V );
				float fresnelNode5_g11 = ( _FresnelTransBias + clampResult2_g11 * pow( max( 1.0 - fresnelNdotV5_g11 , 0.0001 ), _FresnelTransPower ) );
				float temp_output_7_0_g11 = saturate( ( 1.0 - fresnelNode5_g11 ) );
				float OpacityFresnelSub743 = temp_output_7_0_g11;
		...
		Pass
		{
					 ...
					 )
			{
				...
				float2 uv_DetailMap = UV0.xy * _DetailMap_ST.xy + _DetailMap_ST.zw;
				float TexturesScale451 = _TexturesScale;
				float4 unityObjectToClipPos420 = TransformWorldToHClip( TransformObjectToWorld( ( packedInput.ase_texcoord8.xyz ).xyz ) );
				float4 computeScreenPos421 = ComputeScreenPos( unityObjectToClipPos420 , _ProjectionParams.x );
				float4 unityObjectToClipPos437 = TransformWorldToHClip( TransformObjectToWorld( ( float3( 0, 0, 0 ) ).xyz ) );
				float4 computeScreenPos438 = ComputeScreenPos( unityObjectToClipPos437 , _ProjectionParams.x );
				float4 transform428 = mul(GetObjectToWorldMatrix(),float4( PositionWS , 0.0 ));
